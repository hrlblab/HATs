...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.avg_pool = nn.AvgPool2d(kernel_size=(2, 1), stride=2)
    ...
    def forward(self, x: torch.Tensor, task_id, scale_id) -> torch.Tensor:
        ...
        now_cls_token = self.cls_emb[:,task_id[0].long(),:].unsqueeze(1)
        now_sls_token = self.sls_emb[:,scale_id[0].long(),:].unsqueeze(1)
        ...
        return x, now_cls_token, now_sls_token
