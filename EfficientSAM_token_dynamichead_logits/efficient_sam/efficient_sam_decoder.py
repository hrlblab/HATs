...
class MaskDecoder(nn.Module):
    ...
    def forward(
        ...
        #     sparse_prompt_embeddings.shape[1], sparse_embed_dim_1, sparse_embed_dim_2
        # )

        hyper_in, upscaled_embedding, iou_pred = self.predict_masks(
        ...
        if multimask_output and self.num_multimask_outputs > 1:
            ...
            return hyper_in, upscaled_embedding, iou_pred[:, 1:]
        else:
            return hyper_in, upscaled_embedding, iou_pred[:, :1]
    ...
    def predict_masks(
        ...
        'return upscaled_embedding here for dynamic head in efficient_sam.py'
        # masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w)
        # Generate mask quality predictions
        iou_pred = self.iou_prediction_head(iou_token_out)
        return 0, upscaled_embedding, iou_pred
